...
# Combine datasets
combined_data_cortex <- bind_rows(Left_Cortex_long, Right_Cortex_long) %>%
  mutate(Location = rep(c("Left Cortex", "Right Cortex"), each = 10))
...
# Plot
gg_cortex <- ggplot(combined_data_cortex, aes(x = Time, y = Signal, fill = Agent)) +
...
ggsave("output_plot_cortex.png", plot = gg_cortex, width = 9000, height = 2500, units = "px")
=======
library(ggplot2)
library(dplyr)
library(tidyr)
