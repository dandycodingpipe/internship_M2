...
# Combine datasets
combined_data_medulla <- bind_rows(Left_Medulla_long, Right_Medulla_long) %>%
  mutate(Location = rep(c("Left Medulla", "Right Medulla"), each = 10))
...
# Plot
gg_medulla <- ggplot(combined_data_medulla, aes(x = Time, y = Signal, fill = Agent)) +
...
ggsave("output_plot_medulla.png", plot = gg_medulla, width = 9000, height = 2500, units = "px")
=======
library(ggplot2)
library(dplyr)
library(tidyr)
